...
import { PlusOutlined, EditOutlined, CheckOutlined, CloseOutlined } from '@ant-design/icons-vue';
...
const incomeLimit = computed(() => {
  ...
  const selectedOption = uitOptions.find(opt => opt.year === selectedYear.value);
  if (!selectedOption) return 0;
  return (selectedOption.value * 7) / (1 - 0.20); // Ajuste para suspensión.
});
...
const refreshRetentions = () => {
  if (suspensionEnabled.value) {
    ...
    detailedIncomes.value.forEach(item => {
      const currentVal = Number(item.value) || 0;
      item.retention = (currentVal > 1500) ? parseFloat((currentVal * 0.08).toFixed(2)) : 0;
    });
  }
    ...
    if (suspensionEnabled.value) {
      if (cumulative < incomeLimit.value) {
        if (cumulative + currentVal <= incomeLimit.value) {
          ...
          if (i === index) newRetention = parseFloat((currentVal * 0.08).toFixed(2));
        }
      } else {
        if (i === index) newRetention = parseFloat((currentVal * 0.08).toFixed(2));
      }
      ...
      if (i === index) {
        newRetention = (currentVal > 1500) ? parseFloat((currentVal * 0.08).toFixed(2)) : 0;
      }
    }
  }
...
const onDelete = (key) => {
  detailedIncomes.value = detailedIncomes.value.filter(item => item.key !== key);
  refreshRetentions();
};

const totalIncome = computed(() => {
  return detailedIncomes.value.reduce((sum, item) => sum + (Number(item.value) || 0), 0);
});

const totalRetentions = computed(() => {
  return detailedIncomes.value.reduce((sum, item) => sum + (Number(item.retention) || 0), 0);
});
...
const deduccion20PorCiento = computed(() => {
  ...
  const base = rentaBruta4ta.value * 0.20;
  return base > maxDeducible ? maxDeducible : parseFloat(base.toFixed(2));
});
const rentaNeta4ta = computed(() => {
  return parseFloat((rentaBruta4ta.value - deduccion20PorCiento.value).toFixed(2));
});

// Total 4ta + 5ta
const total4ta5ta = computed(() => {
  // Suma de renta neta 4ta + renta bruta 5ta + otras rentas
  return parseFloat((rentaNeta4ta.value + rentaBruta5ta.value + Number(otrasRentas.value || 0)).toFixed(2));
});
...
const totalRentaNeta = computed(() => {
  ...
  const result = total4ta5ta.value - deduccion7UIT.value - gastosDeducibles.value;
...
const bracketCalculation = computed(() => {
  ...
  const bracketDefs = [
    ...
    { limitUIT: 45, rate: 0.20 }, // Exceso de 35 UIT hasta 45 UIT
    { limitUIT: Infinity, rate: 0.30 } // Exceso de 45 UIT
  ];
  ...
  for (let i = 0; i < bracketDefs.length; i++) {
    ...
    if (i === 0) {
      ...
      rangeLabel = `Exceso de ${bracketDefs[i - 1].limitUIT} UIT hasta ${bracketDefs[i].limitUIT} UIT`;
    }
...
const totalImpuesto = computed(() => {
  return bracketCalculation.value.reduce((sum, item) => {
...
const impuestoRegularizar = computed(() => {
  const totalRet = Number(saldoFavorAnterior.value)
...
--- IMPUESTO POR TRAMOS ---
${bracketCalculation.value.map((b, idx) =>
    `Tramo ${idx + 1}: ${b.alcance} => ${b.tramo} soles a ${b.tasa} => Impuesto: S/. ${b.impuesto}`
  ).join('\n')}
      ...
      <div class="mx-auto max-w-7xl sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="bg-white shadow-sm sm:rounded-lg dark:bg-gray-800 p-6">
          <div class="flex items-center justify-between space-x-4">
            ...
            <div v-if="selectedUIT" class="ml-4 flex items-center space-x-4">
              <!-- Muestra el valor del UIT -->
              ...
              <!-- Muestra el decreto -->
              <span class="text-gray-600">{{ selectedUIT.decree }}</span>
            </div>
          </div>
        </div>

        <!-- Dos columnas: la izquierda (más ancha) y la derecha (más angosta) -->
          ...
          <div class="space-y-6">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg dark:bg-gray-800 p-6">
              ...
              <p class="text-gray-700 dark:text-gray-300 mt-2">
                La renta de cuarta categoría corresponde a los ingresos obtenidos por el trabajo independiente
                de profesionales, técnicos y otros trabajadores independientes que emiten recibos por honorarios.
              </p>

              <!-- Campos de ingresos totales -->
              <div class="mt-4 space-y-4">
                <div class="flex items-center space-x-4">
                  <span class="text-gray-900 dark:text-gray-100 font-medium">Renta bruta</span>
                  <a-input
                    :value="totalIncome"
                    disabled
                    class="w-40"
                ...
                <div class="flex items-center space-x-4">
                  <span class="text-gray-900 dark:text-gray-100 font-medium">Retención total</span>
                  <a-input
                ...
                <div v-if="selectedYear" class="flex items-center space-x-4">
              ...
              <a-button type="link" class="mt-4" @click="showDetailed = !showDetailed">
                {{ showDetailed ? 'Ocultar detalles' : 'Desglosar ingresos' }}
              </a-button>
              ...
              <div v-if="showDetailed" class="mt-4">
                <div class="flex items-center justify-between">
                  <div class="flex items-center space-x-2">
                    <a-button type="primary" @click="addIncome">
                  ...
                  <div class="flex items-center space-x-2">
                    <span class="text-gray-900 dark:text-gray-100">Suspensión de retención (8%)</span>
                    <a-switch v-model:checked="suspensionEnabled" @change="refreshRetentions" />
                  </div>
                </div>
                ...
                <a-table
                  ...
                  :columns="[
                    ...
                    { title: 'Valor (S/)', dataIndex: 'value', key: 'value' },
                    { title: 'Retención (S/)', dataIndex: 'retention', key: 'retention' },
                ...
                >
                  <template #bodyCell="{ column, record }">
                      ...
                      <div class="editable-cell relative">
                        <div v-if="editableData[record.key]" class="editable-cell-input-wrapper">
                          <a-input-number
                            ...
                            @change="updateEditableRetention(record.key)"
                            @pressEnter="save(record.key)"
                            class="w-full"
                          />
                          <CheckOutlined
                    ...
                    <!-- Columna Retención -->
                    <template v-else-if="column.dataIndex === 'retention'">
                      <span>{{ record.retention }}</span>
                    </template>

                    <!-- Columna para borrar -->
                    <template v-else-if="column.dataIndex === 'operation'">
                      <a-popconfirm title="¿Seguro de eliminar?" @confirm="() => onDelete(record.key)">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg dark:bg-gray-800 p-6">
              ...
              <p class="text-gray-700 dark:text-gray-300 mt-2">
                Ingrese su remuneración anual o desglose su sueldo mensual. El monto retenido se calculará como
                (Remuneración anual – 7 UIT) * 0.08.
              </p>

              <div class="mt-4 space-y-4">
                <!-- Opción para desglosar sueldo mensual -->
                <div class="flex items-center space-x-4">
                  <span class="text-gray-900 dark:text-gray-100 font-medium">Desglosar sueldo por mes</span>
                  <a-switch v-model:checked="showMonthlyBreakdown" />
                </div>
                <!-- Remuneración anual -->
                <div class="flex items-center space-x-4">
                  <span class="text-gray-900 dark:text-gray-100 font-medium">Remuneración anual</span>
                ...
                <div v-if="showMonthlyBreakdown" class="grid grid-cols-3 gap-4">
                  <div v-for="(month, index) in months" :key="month" class="flex items-center space-x-2">
                    <span class="text-gray-900 dark:text-gray-100">{{ month }}:</span>
                    <a-input-number
                      v-model:value="monthlySalaries[index]"
                      placeholder="0"
                      class="w-32"
                      type="number"
                    >
                ...
                <!-- Límite anual (7 UIT) -->
                <div class="flex items-center space-x-4">
                  <span class="text-gray-900 dark:text-gray-100 font-medium">Límite anual (7 UIT)</span>
                  <a-input
                ...
                <!-- Monto retenido -->
                <div class="flex items-center space-x-4">
                  <span class="text-gray-900 dark:text-gray-100 font-medium">Monto retenido</span>
                  <a-input
            ...
            <div class="bg-white shadow-sm sm:rounded-lg dark:bg-gray-800 p-6">
              ...
              <p class="text-gray-700 dark:text-gray-300 mt-2">
                Ingrese los montos anuales de gastos en cada categoría. Se aplica el porcentaje indicado a cada uno,
                y la suma total de deducciones no podrá exceder 3 UIT.
              </p>
              ...
              <a-table
                ...
                :columns="[
                  ...
                  { title: 'Gasto total anual (S/)', dataIndex: 'annualSpending', key: 'annualSpending' },
                  { title: 'Porcentaje a deducir', dataIndex: 'deductionRate', key: 'deductionRate' },
                  { title: 'Valor a deducir (S/)', dataIndex: 'valueToDeduct', key: 'valueToDeduct' }
                ]"
                :pagination="false"
                bordered
                class="mt-4"
              >
                <template #bodyCell="{ column, record }">
                  ...
                  <template v-else-if="column.dataIndex === 'annualSpending'">
                    <a-input-number
                      v-model:value="record.annualSpending"
                      placeholder="0"
                      class="w-full"
                    >
                      <template #prefix>S/.</template>
                  ...
                  <template v-else-if="column.dataIndex === 'deductionRate'">
                    {{ (record.deductionRate * 100) }}%
                  </template>

                  <!-- Valor a deducir (parcial, sin el tope) -->
                  <template v-else-if="column.dataIndex === 'valueToDeduct'">
                    {{ ((record.annualSpending || 0) * record.deductionRate).toFixed(2) }}
                  </template>
                </template>
              </a-table>
          ...
          <div class="space-y-6">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg dark:bg-gray-800 p-6">
              ...
              <div class="mt-4 grid grid-cols-1 gap-4">
                ...
                <div class="flex items-center space-x-4 justify-between">
                  <span class="text-gray-900 dark:text-gray-100 font-medium">Renta Bruta 4ta</span>
                  <a-input :value="rentaBruta4ta" disabled class="w-40" />
                </div>
                ...
                <div class="flex items-center space-x-4 justify-between">
                  ...
                  <a-input :value="deduccion20PorCiento" disabled class="w-40" />
                </div>

                <!-- Renta Neta 4ta -->
                <div class="flex items-center space-x-4 justify-between">
                  <span class="text-gray-900 dark:text-gray-100 font-medium">Renta Neta 4ta</span>
                  <a-input :value="rentaNeta4ta" disabled class="w-40" />
                </div>

                <!-- Total Renta 5ta (bruta) -->
                <div class="flex items-center space-x-4 justify-between">
                  <span class="text-gray-900 dark:text-gray-100 font-medium">Renta 5ta</span>
                  <a-input :value="rentaBruta5ta" disabled class="w-40" />
                </div>

                <!-- Otras Rentas (4ta o 5ta) (Editable) -->
                <div class="flex items-center space-x-4 justify-between">
                  <span class="text-gray-900 dark:text-gray-100 font-medium">Otras Rentas</span>
                  <a-input v-model:value="otrasRentas" class="w-40" />
                </div>

                <!-- Total Rentas de 4ta y 5ta (sumado) -->
                <div class="flex items-center space-x-4 justify-between">
                  <span class="text-gray-900 dark:text-gray-100 font-medium">Total 4ta + 5ta</span>
                  <a-input :value="total4ta5ta" disabled class="w-40" />
                </div>

                <!-- Deducción 7 UIT -->
                <div class="flex items-center space-x-4 justify-between">
                  <span class="text-gray-900 dark:text-gray-100 font-medium">Deducción 7 UIT</span>
                  <a-input :value="deduccion7UIT" disabled class="w-40" />
                </div>
                ...
                <div class="flex items-center space-x-4 justify-between">
                  ...
                  <a-input :value="gastosDeducibles" disabled class="w-40" />
                </div>

                <!-- ITF (Editable) -->
                <div class="flex items-center space-x-4 justify-between">
                  <span class="text-gray-900 dark:text-gray-100 font-medium">ITF</span>
                  <a-input v-model:value="itf" class="w-40" />
                </div>

                <!-- Total Renta Neta de Trabajo -->
                <div class="flex items-center space-x-4 justify-between">
                  <span class="text-gray-900 dark:text-gray-100 font-medium">Total Renta Neta</span>
                  <a-input :value="totalRentaNeta" disabled class="w-40" />
                </div>
                ...
                <div class="flex items-center space-x-4 justify-between">
                  ...
                  <a-input v-model:value="rentaExtranjera" class="w-40" />
                </div>
                ...
                <div class="flex items-center space-x-4 justify-between">
                  ...
                  <a-input :value="totalRentaImponible" disabled class="w-40" />
                </div>
              </div>
            </div>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg dark:bg-gray-800 p-6">
              ...
              <a-table
                ...
                :columns="[
                  { title: 'Alcance', dataIndex: 'alcance', key: 'alcance' },
                  { title: 'Tasa', dataIndex: 'tasa', key: 'tasa' },
                  { title: 'Tramo (S/)', dataIndex: 'tramo', key: 'tramo' },
                  { title: 'Impuesto (S/)', dataIndex: 'impuesto', key: 'impuesto' }
                ]"
                :pagination="false"
                bordered
                class="mt-4"
            ...
            <div class="bg-white shadow-sm sm:rounded-lg dark:bg-gray-800 p-6">
              ...
              <div class="mt-4 space-y-4">
                ...
                <div class="flex items-center space-x-4">
                  ...
                  <a-input v-model:value="saldoFavorAnterior" class="w-40" />
                </div>
                ...
                <div class="flex items-center space-x-4">
                  ...
                  <a-input v-model:value="pagosCuentaCuarta" class="w-40" />
                </div>
                ...
                <div class="flex items-center space-x-4">
                  ...
                  <a-input :value="retencionesCuarta" disabled class="w-40" />
                </div>

                <!-- Retenciones por quinta categoría (Planilla) (No editable) -->
                <div class="flex items-center space-x-4">
                  <span class="text-gray-900 dark:text-gray-100 font-medium">
                    Retenciones por quinta categoría (Planilla)
                  </span>
                  <a-input :value="retencionesQuinta" disabled class="w-40" />
                </div>
                ...
                <div class="flex items-center space-x-4">
                  ...
                  <a-input :value="impuestoRegularizar" disabled class="w-40" />
                </div>
              </div>
            </div>
